...
class FragmentationTracker:
    ...
    def process_event(self, cpu, data, size):
        ...
        if event.event_type == 3:  # EXTFRAG event
            ...
            from_type = MIGRATE_TYPES.get(event.migrate_from, f"TYPE_{event.migrate_from}")
            ...
            if self.verbose:
                ...
                print(f"{color}[{rel_time:8.3f}s] {event_name:10s}\033[0m "
            ...
            zone_name = self.zone_names[event.zone_idx] if 0 <= event.zone_idx < len(self.zone_names) else "Unknown"
    ...
    def print_summary(self):
        ...
        if total_events > 0:
            # Count by type
            extfrag_count = sum(1 for e in self.events_data if e['event_type'] == 'extfrag')
            compact_success = sum(1 for e in self.events_data if e['event_type'] == 'compaction_success')
            compact_fail = sum(1 for e in self.events_data if e['event_type'] == 'compaction_failure')
            ...
            if extfrag_count > 0:
                ...
                extfrag_events = [e for e in self.events_data if e['event_type'] == 'extfrag']
                ...
                print("-"*40)
                for pattern, count in sorted(migrate_patterns.items(), key=lambda x: x[1], reverse=True)[:5]:
                    print(f"  {pattern:<30} {count:5d} ({count/extfrag_count*100:5.1f}%)")
                
                print(f"\nSteal vs Claim:")
                print(f"  Steal (partial): {steal_vs_claim['steal']} ({steal_vs_claim['steal']/extfrag_count*100:.1f}%)")
                print(f"  Claim (whole):   {steal_vs_claim['claim']} ({steal_vs_claim['claim']/extfrag_count*100:.1f}%)")
            
            # Compaction analysis
            if self.compact_stats:
                print("\nCompaction Events by Order:")
                print("-"*40)
                print(f"{'Order':<8} {'Success':<10} {'Failure':<10} {'Total':<10} {'Success%':<10}")
                print("-"*40)
                ...
                for order in sorted(self.compact_stats.keys()):
                    ...
                    print(f"{order:<8} {stats['success']:<10} {stats['failure']:<10} "
    ...
    def run(self):
        ...
        has_compaction = os.path.exists('/sys/kernel/debug/tracing/events/page_alloc/mm_compaction_success')
...
def main():
    parser = argparse.ArgumentParser(description='Track memory fragmentation events using eBPF')
    parser.add_argument('-o', '--output', help='Output JSON file')
    parser.add_argument('-t', '--time', type=int, help='Run for specified seconds then exit')
    parser.add_argument('-q', '--quiet', action='store_true', help='Suppress event output (summary only)')
