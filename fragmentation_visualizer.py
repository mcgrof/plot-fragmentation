...
def create_overlaid_compaction_graph(ax, data_a, data_b, labels):
    ...
    compact_a = [e for e in events_a if e['event_type'] in ['compaction_success', 'compaction_failure']]
    ...
    compact_b = [e for e in events_b if e['event_type'] in ['compaction_success', 'compaction_failure']]
    ...
    # Plot A with circles
    for e in success_a:
        ax.scatter(e['timestamp'], e.get('fragmentation_index', 0), 
                  s=get_dot_size(e['order']), c='#2ecc71', 
                  alpha=0.3, edgecolors='none', marker='o', label=None)
    
    for i, e in enumerate(failure_a):
        y_pos = -50 - (e['order'] * 10)
        ax.scatter(e['timestamp'], y_pos, 
                  s=get_dot_size(e['order']), c='#e74c3c', 
                  alpha=0.3, edgecolors='none', marker='o', label=None)
    
    # Plot B with triangles
    for e in success_b:
        ax.scatter(e['timestamp'], e.get('fragmentation_index', 0), 
                  s=get_dot_size(e['order'])*1.2, c='#27ae60', 
    ...
    for i, e in enumerate(failure_b):
        ...
        ax.scatter(e['timestamp'], y_pos, 
                  s=get_dot_size(e['order'])*1.2, c='#c0392b', 
                  alpha=0.4, edgecolors='black', linewidths=0.5,
                  marker='^', label=None)
    ...
    all_y_values = []
    if success_a:
        all_y_values.extend([e.get('fragmentation_index', 0) for e in success_a if e.get('fragmentation_index', 0) <= 1000])
    if success_b:
        all_y_values.extend([e.get('fragmentation_index', 0) for e in success_b if e.get('fragmentation_index', 0) <= 1000])
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='#2ecc71', 
               markersize=8, alpha=0.6, label=f'{labels[0]} Success'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='#e74c3c', 
               markersize=8, alpha=0.6, label=f'{labels[0]} Failure'),
        Line2D([0], [0], marker='^', color='w', markerfacecolor='#27ae60', 
               markersize=8, alpha=0.6, label=f'{labels[1]} Success'),
        Line2D([0], [0], marker='^', color='w', markerfacecolor='#c0392b', 
               markersize=8, alpha=0.6, label=f'{labels[1]} Failure'),
    ]
    # Position legend at y=0 on the left side where there's no data
    ax.legend(handles=legend_elements, loc='center left', 
              bbox_to_anchor=(0.02, 0.5), ncol=1, fontsize=8, 
    ...
    ax.set_title('Compaction Events Comparison (○ = A, △ = B)', fontsize=13, fontweight='bold', pad=20)
...
def create_overlaid_extfrag_timeline(ax, data_a, data_b, labels, bin_size=0.5):
    ...
    if steal_times_a.size > 0:
        ax.plot(steal_times_a, steal_counts_a, linewidth=2, color='#3498db', 
                alpha=0.8, label=f'{labels[0]} Steal', linestyle='-')
        ax.fill_between(steal_times_a, 0, steal_counts_a, alpha=0.15, color='#3498db')
    
    if claim_times_a.size > 0:
        ax.plot(claim_times_a, claim_counts_a, linewidth=2, color='#e67e22', 
                alpha=0.8, label=f'{labels[0]} Claim', linestyle='-')
    ...
    if steal_times_b.size > 0:
        ax.plot(steal_times_b, steal_counts_b, linewidth=2, color='#2980b9', 
                alpha=0.8, label=f'{labels[1]} Steal', linestyle='--')
    
    if claim_times_b.size > 0:
        ax.plot(claim_times_b, claim_counts_b, linewidth=2, color='#d35400', 
                alpha=0.8, label=f'{labels[1]} Claim', linestyle='--')
    
    ax.legend(loc='upper right', frameon=True, fontsize=9, ncol=2)
    ax.set_xlabel('Time (seconds)', fontsize=11)
    ax.set_ylabel(f'Events per {bin_size}s', fontsize=11)
    ax.set_title('ExtFrag Events Timeline (Solid = A, Dashed = B)', fontsize=12, fontweight='semibold')
...
def create_combined_migration_heatmap(ax, data_a, data_b, labels):
    ...
    if not events_a and not events_b:
        ax.text(0.5, 0.5, 'No external fragmentation events', 
    ...
    sorted_patterns = sorted(all_patterns, 
                           key=lambda p: (pattern_severities[p], p))
    ...
    for i in range(len(sorted_patterns)):
        for j in range(len(time_centers)):
            ...
            rect = Rectangle((j-0.5, i-0.5), 1, 1, 
                           facecolor=base_color, alpha=0.1, 
                           edgecolor='gray', linewidth=0.5)
            ax.add_patch(rect)
            
            # Add counts for A (upper half)
            if heatmap_a[i, j] > 0:
                rect_a = Rectangle((j-0.4, i), 0.8, 0.4, 
                                  facecolor='#3498db', alpha=0.6)
                ax.add_patch(rect_a)
                ax.text(j, i+0.2, str(int(heatmap_a[i, j])), 
                       ha='center', va='center', fontsize=6, 
                       color='white', fontweight='bold')
            
            # Add counts for B (lower half)
            if heatmap_b[i, j] > 0:
                rect_b = Rectangle((j-0.4, i-0.4), 0.8, 0.4, 
                                  facecolor='#e67e22', alpha=0.6)
                ax.add_patch(rect_b)
                ax.text(j, i-0.2, str(int(heatmap_b[i, j])), 
                       ha='center', va='center', fontsize=6, 
                       color='white', fontweight='bold')
    ...
    for i, pattern in enumerate(sorted_patterns):
        ...
        rect = Rectangle((-1.8, i - 0.4), 1.0, 0.8,
                        facecolor=severity_color, alpha=0.8,
    ...
    ax.legend(handles=legend_elements, loc='upper right', 
              bbox_to_anchor=(1.15, 1.0), fontsize=8, frameon=True)
    
    # Styling
    ax.set_xlabel('Time', fontsize=11)
    ax.set_ylabel('Migration Pattern', fontsize=11)
    ax.set_title('Migration Patterns Comparison (Blue = A, Orange = B)', 
...
def create_comparison_statistics_table(ax, data_a, data_b, labels):
    ...
    def calculate_metrics(data):
        ...
        compact = [e for e in events if e['event_type'] in ['compaction_success', 'compaction_failure']]
        extfrag = [e for e in events if e['event_type'] == 'extfrag']
        
        compact_success = sum(1 for e in compact if e['event_type'] == 'compaction_success')
        success_rate = (compact_success / len(compact) * 100) if compact else 0
        
        bad = sum(1 for e in extfrag 
                 if get_migration_severity(e['migrate_from'], e['migrate_to']) < 0)
        good = sum(1 for e in extfrag 
                  if get_migration_severity(e['migrate_from'], e['migrate_to']) > 0)
    ...
    rows = [
        ['Total Events', metrics_a['total'], metrics_b['total'], 
         '=' if metrics_a['total'] == metrics_b['total'] else ''],
        ['Compaction Success Rate', f"{metrics_a['compact_success_rate']:.1f}%", 
         f"{metrics_b['compact_success_rate']:.1f}%",
         labels[0] if metrics_a['compact_success_rate'] > metrics_b['compact_success_rate'] 
         else labels[1] if metrics_b['compact_success_rate'] > metrics_a['compact_success_rate'] else '='],
        ['ExtFrag Events', metrics_a['extfrag'], metrics_b['extfrag'],
         labels[0] if metrics_a['extfrag'] < metrics_b['extfrag'] 
         else labels[1] if metrics_b['extfrag'] < metrics_a['extfrag'] else '='],
        ['Bad Migrations', metrics_a['bad_migrations'], metrics_b['bad_migrations'],
         labels[0] if metrics_a['bad_migrations'] < metrics_b['bad_migrations'] 
         else labels[1] if metrics_b['bad_migrations'] < metrics_a['bad_migrations'] else '='],
        ['Good Migrations', metrics_a['good_migrations'], metrics_b['good_migrations'],
         labels[0] if metrics_a['good_migrations'] > metrics_b['good_migrations'] 
         else labels[1] if metrics_b['good_migrations'] > metrics_a['good_migrations'] else '='],
    ...
    table = ax.table(cellText=rows, colLabels=headers,
                    ...
                    bbox=[0.1, 0.15, 0.8, 0.65])  # Center table with margins
    ...
    ax.set_title('\n\nStatistical Comparison (Green = Better, Red = Worse)', 
                fontsize=13, fontweight='bold', pad=5, y=1.0)
...
def create_single_dashboard(data, output_file=None, bin_size=0.5):
    ...
    gs = gridspec.GridSpec(3, 1, height_ratios=[2.5, 2, 3], 
    ...
    # Process events
    events = data.get('events', [])
    compact_events = [e for e in events if e['event_type'] in ['compaction_success', 'compaction_failure']]
    extfrag_events = [e for e in events if e['event_type'] == 'extfrag']
    
    success_events = [e for e in compact_events if e['event_type'] == 'compaction_success']
    failure_events = [e for e in compact_events if e['event_type'] == 'compaction_failure']
    
    # === COMPACTION GRAPH ===
    if compact_events:
        for e in success_events:
            if e.get('fragmentation_index', 0) <= 1000:  # Cap at 1000
                ax_compact.scatter(e['timestamp'], e.get('fragmentation_index', 0), 
                                 s=get_dot_size(e['order']), c='#2ecc71', 
                                 alpha=0.3, edgecolors='none')
        
        for i, e in enumerate(failure_events):
            y_pos = -50 - (e['order'] * 10)
            ax_compact.scatter(e['timestamp'], y_pos, 
                             s=get_dot_size(e['order']), c='#e74c3c', 
                             alpha=0.3, edgecolors='none')
        
        ax_compact.axhline(y=0, color='#34495e', linestyle='-', linewidth=1.5, alpha=0.8)
        ax_compact.grid(True, alpha=0.08, linestyle=':', linewidth=0.5)
        ax_compact.set_ylim(-200, 1000)
        
        # Add statistics
        success_rate = len(success_events) / len(compact_events) * 100 if compact_events else 0
        stats_text = f"Success: {len(success_events)}/{len(compact_events)} ({success_rate:.1f}%)"
        ax_compact.text(0.02, 0.98, stats_text, transform=ax_compact.transAxes,
    ...
    if extfrag_events:
        ...
        if steal_times.size > 0:
            ax_extfrag.fill_between(steal_times, 0, steal_counts, alpha=0.3, color='#3498db')
            ax_extfrag.plot(steal_times, steal_counts, linewidth=2, color='#2980b9', 
                          alpha=0.8, label=f'Steal ({len(steal_events)})')
        
        if claim_times.size > 0:
            ax_extfrag.fill_between(claim_times, 0, claim_counts, alpha=0.3, color='#e67e22')
            ax_extfrag.plot(claim_times, claim_counts, linewidth=2, color='#d35400', 
                          alpha=0.8, label=f'Claim ({len(claim_events)})')
        
        ax_extfrag.legend(loc='upper right', frameon=True, fontsize=9)
        
        # Add bad/good migration counts
        bad_migrations = sum(1 for e in extfrag_events 
                           if get_migration_severity(e['migrate_from'], e['migrate_to']) < 0)
        good_migrations = sum(1 for e in extfrag_events 
                            if get_migration_severity(e['migrate_from'], e['migrate_to']) > 0)
        
        migration_text = f"Bad: {bad_migrations} | Good: {good_migrations}"
        ax_extfrag.text(0.02, 0.98, migration_text, transform=ax_extfrag.transAxes,
                      fontsize=10, verticalalignment='top',
                      bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.9))
    
    ax_extfrag.set_xlabel('Time (seconds)', fontsize=11)
    ax_extfrag.set_ylabel(f'Events per {bin_size}s', fontsize=11)
    ax_extfrag.set_title('External Fragmentation Events Timeline', fontsize=12, fontweight='semibold')
    ...
    # Super title
    fig.suptitle('Memory Fragmentation Analysis', fontsize=18, fontweight='bold', y=0.98)
    
    # Footer
    timestamp_text = f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    fig.text(0.98, 0.01, timestamp_text, ha='right', fontsize=9, 
...
def create_single_migration_heatmap(ax, extfrag_events):
    ...
    sorted_patterns = sorted(patterns.keys(), 
                           key=lambda p: (patterns[p]['severity'], -patterns[p]['total']))
    ...
    for i, pattern in enumerate(sorted_patterns):
        ...
        rect = Rectangle((-2.3, i - 0.4), 1.5, 0.8,
                        facecolor=severity_color, alpha=0.8,
                        edgecolor='black', linewidth=0.5,
                        clip_on=False)
        ax.add_patch(rect)
    ...
    ax.legend(handles=[bad_patch, neutral_patch, good_patch], 
             loc='upper right', bbox_to_anchor=(1.15, 1.0),
             title='Migration Impact', fontsize=8, title_fontsize=9)
    
    # Styling
    ax.set_xlabel('Time', fontsize=11)
    ax.set_ylabel('Migration Pattern', fontsize=11)
    ax.set_title('Migration Patterns Timeline with Severity Indicators', 
                fontsize=12, fontweight='semibold')
    ax.grid(False)
    ...
    bad_events = sum(patterns[p]['total'] for p in patterns 
                    if patterns[p]['severity'] < 0)
    good_events = sum(patterns[p]['total'] for p in patterns 
                     if patterns[p]['severity'] > 0)
...
def create_comparison_dashboard(data_a, data_b, labels, output_file=None):
    ...
    gs = gridspec.GridSpec(4, 1, height_ratios=[2.5, 2, 2.5, 1.5], 
    ...
    # Super title
    fig.suptitle('Memory Fragmentation A/B Comparison Analysis', 
...
def main():
    parser = argparse.ArgumentParser(description='Fragmentation analysis with optional comparison')
    parser.add_argument('input_file', help='Primary JSON file')
    parser.add_argument('--compare', help='Secondary JSON file for A/B comparison (optional)')
    parser.add_argument('-o', '--output', help='Output filename')
    parser.add_argument('--labels', nargs=2, default=["Light Load", "Heavy Load"],
                       help='Labels for the two datasets in comparison mode')
    parser.add_argument('--bin', type=float, default=0.5, help='Bin size for event counts')
